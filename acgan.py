...
def main(epochs=300,
    ...
    combined = Model(inputs=generator.inputs +
                     biases, outputs=combined_output)
    ...
    classless_training_fn = K.function(
        inputs=[real_label]+combined.inputs, outputs=[],
        updates=Doptim.get_updates(
            loss=K.binary_crossentropy(
                target=real_label, output=combined.output[1]
            ), params=classifier.trainable_weights
        )
    )

    classifier_fn = K.function(
        inputs=classifier.inputs, outputs=classifier.outputs)

    Goptim = keras.optimizers.Adam()
    combined_training_fn = K.function(
        inputs=[class_label, real_label]+combined.inputs, outputs=[],
    ...
    for epoch in range(1, epochs+1):
        ...
        for file_num in train_file_num:
            ...
            for index in range(0, total_len-batch_size, batch_size):
                ...
                aux_y = np.expand_dims(.95*np.ones(batch_size), -1)

                classifier_training_fn([y_train, aux_y, x_train])

                noise = np.random.randn(batch_size, latent_size)
                rdn_cls = np.expand_dims(np.random.randint(
                    low=0, high=num_cls-1, size=batch_size), -1)
